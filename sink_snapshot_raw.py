...
class PostgreSQLProcessorRaw:
    ...
    def _connect(self):
        try:
            ...
            print("Connected to PostgreSQL (data_raw schema)")
        except Exception as e:
    ...
    def _insert_ohlcv(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_ohlcv (
    ...
    def _insert_top_price(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_top_price (
          time, symbol, source,
          bp, bq, ap, aq,
          total_bid, total_ask
        ) VALUES (
    ...
    def _insert_stock_tick(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_tick (
          time, symbol, source,
          price, vol, side
        ) VALUES (
          %s::TIMESTAMP, %s, %s,
    ...
    def _insert_stock_foreign(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_foreign (
          time, symbol, source,
    ...
    def _insert_stock_info(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_info (
          time, symbol, source,
          open, high, low, close,
    ...
    def _insert_price_board(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_price_board (
          time, symbol, source,
    ...
    def _insert_market_index(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_market_index (
    ...
    def _insert_market_index_history(self, item: Dict[str, Any]):
        ...
        sql = """
        INSERT INTO data_raw.snapshot_stock_market_index_history (time, symbol, name, source, value)
...
def run_sink_pipeline_raw():
    kafka_servers = os.getenv('KAFKA_SERVERS', 'localhost:9092')
    input_topic = 'dnse.transform'
    group_id = 'dnse.sink.raw.pipeline'
    ...
    print(f"🚀 Starting RAW Sink Pipeline: Kafka '{input_topic}' -> PostgreSQL data_raw schema")
    print("📝 Mode: WRITE-ONLY (no ON CONFLICT)")
    ...
    try:
        db_processor = PostgreSQLProcessorRaw(pg_params)
    except Exception as e:
    ...
    try:
        print("Starting RAW message processing...")
        for message in consumer:
            try:
                ...
                if success:
                    ...
                    print(f"RAW Sunk to DB: {data_type} - {symbol}")
                else:
                    failed += 1
                ...
                if processed % 100 == 0:
                    print(f"RAW Processed {processed} messages. Success: {successful}, Failed: {failed}")
        ...
        consumer.close()
        db_processor.close()
        print(f"RAW Sink pipeline completed. Processed: {processed}, Success: {successful}, Failed: {failed}")


if __name__ == "__main__":
    run_sink_pipeline_raw()
